...
"""Fig 8 (replacement) — superior-cardiac-branch-selective stimulation of the NEW real-3D
human cervical vagus with the 4x5 ring cuff. Same house style as fig5-8 (full-width render
on top, then data panels), extended with the cuff POSITION x multipolar-CONFIG sweep.

  a  3-D nerve + cuff at the swept positions (SCB orange / trunk blue; render)
  b  fiber-diameter population by class            (realistic human vagus pop)
  c  recruitment vs amplitude by class             (best config)
  d  cuff cross-section: epi + fascicle contours, fibers x {SCB/trunk}x{act/silent}
  e  selective recruitment window  (SCB vs trunk)  (best config, realistic pop)
  f  SCB-to-trunk selectivity bar  + selectivity index
  g  evoked AP propagation along a recruited SCB fiber
  h  selectivity index vs cuff position            (controlled 10 um, 3 configs)
  i  operating amplitude vs cuff position          (controlled 10 um, 3 configs)
  j  config comparison at the best position        (controlled 10 um)

Selectivity index = (SCB% - trunk%)/100 at the operating point (= argmax SCB%-trunk%),
exactly as fig6_rabbit.panel_compare. Panels b-g use the realistic mixed-diameter
population at the best position; h-j use the fixed-10um controlled-spatial-selectivity sweep.
"""
...
RENDER = ROOT / "paper_figs/out/renders/new_human_setup.png"
APZ = ROOT / "paper_figs/out/data/new_human_ap.npz"
OUTP = ROOT / "paper_figs/out/figures/png/fig8_human_selectivity.png"
OUTSUPP = ROOT / "paper_figs/out/figures/png/supp_scb_sweep.png"
...
PHI_DEG = 30.0
BEST_TAG, BEST_CFG = "off15_4x5", "long_tripole"
# 30 mm (tag 4x5) dropped: fat cranial trunk, far upstream where fascicles are intermixed.
...
def load_contours(tag):
    ...
    return np.asarray(d["epi"]), [np.asarray(d[f"endo{i}"]) for i in range(int(d["n_endo"]))]
...
def ap_stim_z(ap):
    """AP-initiation site (mm) = z of the node that depolarizes earliest -> correct 'stim'
    arrow position (the cuff/cathode is not at the fiber's geometric centre)."""
    if ap is None:
        return 0.0
...
def panel_render(ax):
    """Render + scale bar (lower-left, the empty corner). Legend / distance labels are still
    exported separately for hand assembly of panel a — see render_new_human_sweep.make_parts."""
    ax.axis("off")
    if RENDER.exists():
        ...
        ax.text(0.5, 0.5, "render missing (run render_new_human_sweep.py)", ha="center",
...
def panel_thumb(ax, tag, dist, is_best):
    ...
    f = ROOT / f"paper_figs/out/renders/new_human_setup_thumb_{tag}.png"
    if f.exists():
...
def panel_xsec(ax, tag, S, best, thr, branch):
    xy = fiber_xy(tag, S[best]["col"]); on = branch == TARGET; off = ~on
    ...
    R = (np.abs(epi).max() if epi is not None and len(epi) else np.percentile(rad, 98)) + 0.4
    ...
    # real epineurium + fascicle (endoneurium) contours
    ...
    # 5-column ring (one axial row in-plane), evenly 72° apart with the cathode toward the
    # SCB; the longitudinal-tripole guards are in the rows above/below (out of plane).
    ...
    ax.plot([lim - 1.0 - 0.06 * R, lim - 0.06 * R], [-lim + 0.07 * R] * 2, "k-", lw=2.2,
            solid_capstyle="butt")
    ax.text(lim - 0.5 - 0.06 * R, -lim + 0.10 * R, "1 mm", ha="center", va="bottom", fontsize=7.5)
...
def panel_window(ax, S, best, letter="e"):
    ...
    ax.legend(fontsize=8.5, loc="upper left", frameon=False)                       # top-left
    ax.text(0.03, 0.74, f"operating point\non {S[best]['son']:.0f}% / off {S[best]['soff']:.0f}%",
            ...
            bbox=dict(facecolor="white", alpha=0.7, edgecolor="none", pad=1.5))   # below legend
    _letter(ax, letter)


def class_si(thr, branch, typ, col):
    """SI per fiber class: SCB-class recruited vs ALL-trunk recruited, max-margin.
    Amplitude grid spans the ACTIVE column's threshold range so the margin maximum
    is well sampled (an all-column grid undersamples it; converged value matches
    controls_lib used in the S12/S13 controls)."""
...
def panel_pw(ax, letter="d"):
    """Pulse-width comparison (long tripole @ best position, realistic pop): per-class SI at
    100 us vs the clinical 300 us, with the all-SCB operating amplitudes annotated. Shows the
    longer pulse halves the amplitude but does NOT recover small-fiber (B) selectivity."""
    ...
    for lab, f, col in series:
        ...
        bars.append((lab, col, rows)); amps.append((lab, A1["long_tripole"]["amp"] / 1e3))
...
def panel_bar(ax, S, best, letter="f"):
    on, off = S[best]["son"], S[best]["soff"]
    ax.bar([0, 1], [on, off], width=0.62, color=[C_ON_ACT, C_OFF_ACT], edgecolor="none")
    for x, v in [(0, on), (1, off)]:
        ax.text(x, v + 2, f"{v:.0f}%", ha="center", fontsize=9, fontweight="bold")
    ax.set_xticks([0, 1]); ax.set_xticklabels(["on-target\n(SCB)", "off-target\n(trunk)"])
    ax.set_ylabel("% recruited at op."); ax.set_ylim(0, 112); ax.set_xlim(-0.6, 1.6)
    ax.text(0.97, 0.96, f"selectivity index {S[best]['si']:.2f}\nop {S[best]['amp']/1e3:.1f} mA",
            transform=ax.transAxes, ha="right", va="top", fontsize=9.5, color="0.2", fontweight="bold")
    ax.set_title(f"{letter}   SCB selectivity · realistic pop.", fontsize=11, loc="left", fontweight="bold")
...
def panel_amp(ax, A, order, letter="b"):
    ...
    ax.set_ylim(top=ax.get_ylim()[1] * 3.0)                          # headroom so the legend clears the lines
    ...
    ax.text(0.98, 3, "≤3 mA (clinical)", transform=ax.get_yaxis_transform(), ha="right",
            va="bottom", fontsize=9, color="#3a7a3a")                # sits just above the band line
    ax.legend(fontsize=8.2, loc="upper right", frameon=False)
    _letter(ax, letter)
...
def main():
    ...
    # ---- realistic population at the best position (b-f); prefer clinical 300 us ----
    ...
    # ---------- MAIN figure: best result only (render + realistic pop, fig5-8 style) ----------
    ...
    gsa = gs[0, :].subgridspec(2, len(POSITIONS), height_ratios=[3.0, 1.0], hspace=0.24, wspace=0.07)
    panel_render(fig.add_subplot(gsa[0, :]))
    # thumbnails reversed so the row order matches the L-R-flipped main render (cuff steps the
    # same direction in both the big render and the small-multiples strip)
    for j, (tag, dist) in enumerate(reversed(POSITIONS)):
        panel_thumb(fig.add_subplot(gsa[1, j]), tag, dist, tag == BEST_TAG)
    axb = fig.add_subplot(gs[1, 0]); diam_panel(axb, diam, typ, "", "b"); axb.set_title("", loc="left")
    ...
    panel_xsec(fig.add_subplot(gs[1, 2]), BEST_TAG, Pc, BEST_CFG, thr_ctrl, branch_ctrl)
    ...
    # ---------- SUPPLEMENTARY: controlled 10 um sweep (a-c) + pulse-width comparison (d) ----------
    ...
    figs.suptitle("Supplementary — controlled 10 µm cuff-position × config sweep (a–c) and "
                  "pulse-width comparison (d)", fontsize=12, fontweight="bold", y=0.965)
