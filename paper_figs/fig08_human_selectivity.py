...
def panel_xsec(ax, tag, S, best, thr, branch):
    ...
    x1 = lim + 0.05 * R; x0 = x1 - 1.0; y = -lim + 0.07 * R                   # tuck into the empty corner
    ax.plot([x0, x1], [y, y], "k-", lw=2.2, solid_capstyle="butt", clip_on=False)
    ax.text((x0 + x1) / 2, y + 0.03 * R, "1 mm", ha="center", va="bottom", fontsize=9, clip_on=False)
...
def panel_window(ax, S, best, letter="e"):
    ...
    ax.legend(fontsize=10.5, loc="upper left", frameon=False)                       # top-left
...
def panel_amp(ax, A, order, letter="b"):
    ...
    ax.text(0.98, 3, "≤3 mA (clinical)", transform=ax.get_yaxis_transform(), ha="right",
            va="bottom", fontsize=10.5, color="#3a7a3a")             # sits just above the band line
    ax.legend(fontsize=10.5, loc="upper right", frameon=False, bbox_to_anchor=(1.0, 1.016))
    _letter(ax, letter)
