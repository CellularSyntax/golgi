...
def build(sp, AP):
    ...
    pop = np.load(DATA / f"thr_pop_{sp['slug']}.npz", allow_pickle=True)
    pthr = pop["thr_uA"][:, 0]
    ...
    fp.recruit_panel(fig.add_subplot(gs[1, 1]), thr1, diam, typ, nm, "c",
                     legend_anchor=((1.056 if nm == "Human" else 1.028), 0.0))  # ~20 px (swine) / ~40 px (human) right
    ...
    axd.legend(handles=_xh, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=2,
               fontsize=7.6, frameon=False, handletextpad=0.3, columnspacing=1.2)
