...
"""Reciprocity FEM on the human multi-region bundle mesh (human_bundle_mesh.py must run
first). Re-attaches the bundle geom, re-sets the ring-array + refit, injects the 576
full-length branch-clustered fibers, and runs golgi's native per-contact lead-field solve
sampled along them -> human_branch/paths_Ve.npz (Ve_mat[N, n_contacts] + branch labels).
"""
...
from human_bundle_mesh import attach_geom, design_dict, EID, PROJ   # noqa: E402

BRANCH = ROOT / "paper_figs/human3d_out/nerve_paths_branch.npz"
OUT = ROOT / "paper_figs/out/data/human_branch"
...
def main():
    ...
    montages = [{"mid": f"rec{i}", "label": f"rec{i}", "kind": "bipolar",
                 "plus_contact": 2 * i, "minus_contact": 2 * i + 1} for i in range(max(1, nc // 2))]
    cfgs = list(s._state.configs)
    for c in cfgs:
        if c.get("design_id") == EID:
            c["recording_montages"] = montages
    s._state.configs = cfgs
    # perineurium as a contact-impedance sheet at the conforming endo<->epi interface
    ...
    print(f"[fem] perineurium CI: thk {peri_thk_m*1e6:.1f}um Rs {peri_thk_m*1149:.4f} ohm.m2", flush=True)
    print(f"[fem] {len(fibers)} fibers, {nc} contacts; reciprocity FEM on bundle mesh ...", flush=True)
    res = s.run_fem()
    ...
    if not cols:
        raise SystemExit(f"no V_e_rec_*.npz in {RECDIR}")
    cids = sorted(cols)
