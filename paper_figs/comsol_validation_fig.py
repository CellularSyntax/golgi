...
def panel_m1(ax):
    ...
    ax.set_title("M1 · monopole in saline", fontsize=10.5, fontweight="bold", loc="left")
    ax.legend(fontsize=7.4, frameon=False, loc="lower left", handlelength=1.8)
...
def panel_scatter(ax, G, C, title, note=""):
    ...
    if note:
        ax.text(0.96, 0.05, note, transform=ax.transAxes, fontsize=6.8, va="bottom",
...
def panel_footprint(ax, G, C, md, note=""):
    ...
    ax.set_title("centre contact · longest fiber", fontsize=9.5, loc="left", color="0.35")
    ax.legend(handles=[l1, l2, l3, l4], fontsize=7.2, frameon=False,
...
def panel_summary(ax):
    ...
    ax.set_xticks(x); ax.set_xticklabels([f"{k}" for k in range(12)], fontsize=8)
    ax.set_xlabel("contact"); ax.set_ylabel("mean |%diff| vs COMSOL")
    ax.set_title("Per-contact lead-field agreement", fontsize=10.5, fontweight="bold", loc="left")
...
def main():
    ...
    gs = GridSpec(3, 2, figure=fig, hspace=0.50, wspace=0.40,
                  top=0.925, bottom=0.05, left=0.075, right=0.905)
    ...
    fig.suptitle("Cross-validation of the open FEniCSx solver against COMSOL",
